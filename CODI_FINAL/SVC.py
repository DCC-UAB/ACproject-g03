# -*- coding: utf-8 -*-
"""
Created on Sun Dec 29 17:08:00 2024
...
print("LinearSVC - Unbalanced Dataset\n")
print(classification_report(y_test_ub, y_pred_ub))
print("LinearSVC - Balanced Dataset\n")
...
axes[0].set_title('LinearSVC - Unbalanced')
...
axes[1].set_title('LinearSVC - Balanced')
plt.show()
...
# Valors de confiança del model (decision_function)
y_score_ub = model.decision_function(X_test_ub)
y_score_b = model.decision_function(X_test_b)

# Gràfica de la corba ROC
plt.figure(figsize=(10, 6))
for i, class_label in enumerate(['negatiu', 'neutre', 'positiu']):
    # Corba ROC per al dataset desbalancejat
    ...
    # Corba ROC per al dataset balancejat
...
# Línia diagonal (random guessing)
plt.plot([0, 1], [0, 1], 'k--', label='Random Guessing')

plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curves - LinearSVC')
plt.legend()
plt.grid()
plt.show()
...
##################################
# EFECTE DEL PARÀMETRE C
##################################

C_values = [0.01, 0.1, 1, 10, 100]
accuracy_C_analysis = {'unbalanced': [], 'balanced': []}

for C in C_values:
    model = LinearSVC(max_iter=10000, C=C)
    model.fit(X_train_ub, y_train_ub)
    accuracy_C_analysis['unbalanced'].append(accuracy_score(y_test_ub, model.predict(X_test_ub)))
    model.fit(X_train_b, y_train_b)
    accuracy_C_analysis['balanced'].append(accuracy_score(y_test_b, model.predict(X_test_b)))

plt.figure(figsize=(8, 5))
plt.plot(C_values, accuracy_C_analysis['unbalanced'], marker='o', label='Unbalanced', linestyle='--')
plt.plot(C_values, accuracy_C_analysis['balanced'], marker='o', label='Balanced')
plt.xscale('log')
