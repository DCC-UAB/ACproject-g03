...
##################################
# EFECTE DE LA PROFUNDITAT MÀXIMA
##################################

max_depth_values = [5, 10, 20, None]
accuracy_max_depth_analysis = {'unbalanced': [], 'balanced': []}

for max_depth in max_depth_values:
    model = DecisionTreeClassifier(max_depth=max_depth, random_state=42)
    model.fit(X_train_ub, y_train_ub)
    accuracy_max_depth_analysis['unbalanced'].append(accuracy_score(y_test_ub, model.predict(X_test_ub)))
    model.fit(X_train_b, y_train_b)
    accuracy_max_depth_analysis['balanced'].append(accuracy_score(y_test_b, model.predict(X_test_b)))

plt.figure(figsize=(8, 5))
plt.plot([str(md) for md in max_depth_values], accuracy_max_depth_analysis['unbalanced'], marker='o', label='Unbalanced', linestyle='--')
plt.plot([str(md) for md in max_depth_values], accuracy_max_depth_analysis['balanced'], marker='o', label='Balanced')
